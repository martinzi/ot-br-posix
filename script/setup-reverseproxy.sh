...
http:
  routers:
    # Well-Known discovery endpoint - publicly accessible, served by C++ backend
    # Priority 110 (higher) ensures exact match takes precedence over PathPrefix
    wellknown-discovery-router:
      rule: "${HOST_RULE} && Path(\`/.well-known/thread\`)"
      entryPoints:
        - websecure
      service: otbr-agent
      tls:
    ...
    # Well-Known certificate endpoints - publicly accessible for enrollment
    # These are served by Python WSGI enrollment service
    # Trailing slash in PathPrefix matches only sub-paths, not root
    wellknown-certs-router:
